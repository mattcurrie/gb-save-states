...
.INCLUDE "includes/packbits_generate_literal.s"
...
/*
    ...
    TODO: use stack instead of variables?


    ; reduce 2 bytes

    ld hl, PACKBITS_CURRENT_OUTPUT_ADDRESS ;   3 bytes / 12 cycles
    ld a,(hl+)  ;   1 byte / 8 cycles
    ld h,(hl)   ;   1 byte / 8 cycles
    ld l,a      ;   1 byte / 8 cycles
 
    total = 6 bytes, 36 cycles


    ld a,(PACKBITS_CURRENT_OUTPUT_ADDRESS)   ; 3 bytes / 16 cycles
    ld l,a                                   ; 1 byte / 4 cycles

    ld a,(PACKBITS_CURRENT_OUTPUT_ADDRESS + 1)  ; 3 bytes / 16 cycles
